...
export class ApplicationsAdministration {
  ...
  public static requestUserApplications(callback: (elements) => void): void {
    $.get(RunPlatformData.URL + "applications/user/outline", (applications) =>
      callback(applications)
    );
  }

  public static requestApplications(
    filters: Array<AppFilter>,
  ...
  public static open(appId: string, callback: Function): void {
    $.get(
      ...
      (application, textStatus) => {
        if (application) {
          callback(JSON.parse(application));
        }
      }
    );
  }

  public static requestUpdateApplication(
    application: any,
    callback: (wsps) => void
  ): void {
    $.ajax({
  ...
  public static requestNewApplication(
    application: any,
    callback: (wsps) => void
  ): void {
    $.ajax({
