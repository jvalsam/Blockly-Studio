...
let CreateEditModal = function(selector){
    let modal = CreateDOMElement('div', {classList: ['modal', 'fade'], id: 'edit-modal'});
    ...
    let modalTitle = CreateDOMElement('h5', {classList: ['modal-title'], id: 'edit-modal-title'});
    ...
    let modalBody = CreateDOMElement('div', {classList: ['modal-body'], id: 'edit-modal-body'});
    modalContent.appendChild(modalBody);

    // Name
    let outterInputDiv = CreateDOMElement('div', {classList: ['input-group', 'mb-3']});
    modalBody.appendChild(outterInputDiv);

    let inputGroup = CreateDOMElement('div', {classList: ['input-group-prepend']});
    outterInputDiv.appendChild(inputGroup);

    let inputGroupNameTag = CreateDOMElement(
                                'div', 
                                {
    ...
    let inputElementName = CreateDOMElement(
                            'input', 
                            {
    ...
    let inputImage = CreateDOMElement(
                        'input', 
                        {
    ...
    // Color
    let outterColorDiv = CreateDOMElement('div', {classList: ['input-group', 'mb-3']});
    modalBody.appendChild(outterColorDiv);

    let inputGroupColor = CreateDOMElement('div', {classList: ['input-group-prepend']});
    outterColorDiv.appendChild(inputGroupColor);

    let inputGroupColorTag = CreateDOMElement(
                                'div', 
                                {
                                    classList: ['input-group-text'], 
    ...
    let inputColor = CreateDOMElement('input', {classList: ['form-control'], id: 'edit-modal-colorInput'});
    ...
    let confirmButton = CreateDOMElement(
                            'div', 
                            {
...
let CreateDeleteModal = function(selector){
    ...
    let modalBody = CreateDOMElement('div', {classList: ['modal-body'], id: 'delete-modal-body'});
    modalContent.appendChild(modalBody);
...
let CreateEditPropertyAliasModal = function(selector){
    ...
    let modalTitle = CreateDOMElement(
                        'h5', 
                        {
    ...
    let inputGroupAliasTag = CreateDOMElement(
                                'div', 
                                {
                                    classList: ['input-group-text'], 
    ...
    let inputPropertyAlias = CreateDOMElement(
                                'input', 
                                {
                                    classList: ['form-control'], 
...
let CreateNewSmartGroupModal = function(selector){
    ...
    let modalTitle = CreateDOMElement('h5', {classList: ['modal-title'], id: 'newGroup-modal-title'});
    modalHeader.appendChild(modalTitle);
    ...
    let modalBody = CreateDOMElement('div', {classList: ['modal-body'], id: 'newGroup-modal-body'});
    modalContent.appendChild(modalBody);
...
let CreateResourceDetails = function(selector, resourceData){
    ...
    // Name in details
    let detailsNameRow = CreateElement('div', {classList: ['row', 'mt-1', 'ml-2', 'bd-highlight']});
    outterDiv.appendChild(detailsNameRow);

    let detailsNameTag = CreateElement('div', {classList: ['col-sm-4', 'font-weight-bold'], innerHtml: 'Name:'});
    detailsNameRow.appendChild(detailsNameTag);

    let detailsName = CreateElement('div', {classList: ['col', 'text-truncate'], innerHtml: resourceData.name});
    //tooltip for name
    ...
    // ID in details
    let detailsIdRow = CreateElement('div', {classList: ['row', 'ml-2', 'bd-highlight']});
    outterDiv.appendChild(detailsIdRow);

    let detailsIdTag = CreateElement('div', {classList: ['col-sm-4', 'font-weight-bold'], innerHtml: 'ID:'});
    detailsIdRow.appendChild(detailsIdTag);

    let detailsId = CreateElement('div', {classList: ['col', 'text-truncate'], innerHtml: resourceData.id});
    //tooltip for id
    ...
    let propertiesRow = CreateElement('div', {classList: ['row', 'mt-1', 'ml-2', 'bd-highlight']});
    outterDiv.appendChild(propertiesRow);

    let propertiesTag = CreateElement('div', {classList: ['col-sm-5', 'font-weight-bold'], innerHtml: 'Properties: '});
    propertiesRow.appendChild(propertiesTag);

    // Table of Properties
    let propertiesTable = CreateElement('table', {classList: ['table-responsive-sm', 'table-striped', 'ml-4', 'mt-3']});
    ...
    _.forIn(resourceData.properties, (prop) => {
        ...
        let propertyName = CreateElement('td', {classList: ['pt-2', 'pb-2'], innerHtml: prop.name});
        propertyRow.appendChild(propertyName);

        let propertyValue = CreateElement('td', {classList: ['pt-2', 'pb-2'], innerHtml: prop.value});
        propertyRow.appendChild(propertyValue);
    });
};

let RenderSOInScanList = function(selector, soData, onRegister){
    let resourceDiv = CreateDOMElement(
                        'div', 
                        {
    ...
    // Card
    let card = CreateDOMElement('div', {classList: ['card', 'list-group-item']});
    ...
    let  buttonCol = CreateDOMElement('div', {classList: ['col-sm-5', 'my-auto']});
    ...
    button.addEventListener('click', () => {onRegister(soData.editorData.details.properties)});
    ...
    // Details
    let detailsRow = CreateDOMElement('div', {classList: ['row', 'h-100', 'd-flex', 'justify-content-center']});
    card.appendChild(detailsRow);

    let resourceDetails = CreateDOMElement('div', {classList: ['fas', 'fa-lg', 'fa-angle-down', 'collapsed']});
    resourceDetails.setAttribute('type', 'button');
    resourceDetails.setAttribute('data-toggle', 'collapse');
    resourceDetails.setAttribute('data-target', soData.editorData.details.id + '-collapse');
    resourceDetails.setAttribute('aria-expanded', 'false');
    resourceDetails.setAttribute('aria-controls', soData.editorData.details.id+'-collapse');
    detailsRow.appendChild(resourceDetails);

    // Collapse for card
    let detailsCollapse = CreateDOMElement('div', {classList: ['collapse', 'mt-2'], id: soData.editorData.id + '-collapse'});
    detailsCollapse.setAttribute('aria-labelledby', soData.editorData.id);
    detailsCollapse.setAttribute('data-parent', soData.editorData.id + '-accordion');
    card.appendChild(detailsCollapse);

    let detailsBody = CreateDOMElement('div', {classList: ['card-body', 'd-flex', 'flex-column', 'bd-highlight', 'bg-light']});
...
export function RenderScanList(selector, arraySOData, onRegister){
    ...
    // Create div for list
    let listDiv = CreateDOMElement('div', {classList: ['list-group', 'scan-list']});
...
let RenderSmartObjectProperty = function(selector, property, alias, callbacks){
    let propertyRow = CreateDOMElement('div', {classList: ['row', 'align-items-center', 'resource-property']});
    selector.appendChild(propertyRow);

    let propertyNameCol = CreateDOMElement(
                            'div', 
                            {
    ...
    let propertyValue = CreateDOMElement('input', {classList: ['form-control', 'property-value-string']});
    ...
    let propertyHideIcon = CreateDOMElement('i', {classList: ['far', 'fa-eye', 'fa-lg']});
    propertyHideIcon.addEventListener('click', () => { callbacks.onEditPropertyProgrammingActive(property) });
    ...
    let propertyEditIcon = CreateDOMElement('i', {classList: ['fas', 'fa-edit', 'fa-lg']});
    propertyEditIcon.addEventListener('click', () => { callbacks.onEditPropertyAlias(property) });
    propertyEditCol.appendChild(propertyEditIcon);
};

let RenderSmartGroupofObject = function(selector, group, onDeleteGroup){
    let groupName = CreateDOMElement(
                        'span', 
                        {
    ...
    let groupIcon = CreateDOMElement('i', {classList: ['fas', 'fa-times-circle']});
    groupIcon.addEventListener('click', () => { onDeleteGroup(group.elemData.name) });
    groupIconSpan.appendChild(groupIcon);
};

let RenderSmartObjectRegistered = function (selector, soData, callbacksMap) {
    let cardDiv = CreateDOMElement('div', {classList: ['card', 'text-left'], id: soData.editorData.id});
    ...
    let dropDownImg = CreateDOMElement(
                        'i', 
                        {
                            classList: ['dropdown-toggle', 'float-right', 'resource-menu', 'fas', 'fa-ellipsis-v'], 
    ...
    let editSelect = CreateDOMElement(
                        'a', 
                        {
    ...
    let deleteSelect = CreateDOMElement(
                            'a', 
                            {
                                classList: ['dropdown-item'], 
    ...
    // Card Body
    let cardBodyDiv = CreateDOMElement(
                        'div', 
                        {
                            classList: ['card-body'], 
    ...
    let environmentName = CreateDOMElement(
                            'div', 
                            {
    ...
    let environmentSelectCol = CreateDOMElement('div', {classList: ['col-sm-6']});
    environmentRow.appendChild(environmentSelectCol);

    let environmentSelect = CreateDOMElement('select', {classList: ['form-control']});
    ...
    let propertiesHeader = CreateDOMElement(
                                'span', 
                                {
                                    classList: ['font-weight-bold'], 
                                    innerHtml: 'Properties (' + soData.editorData.details.properties.length + ')'
                                }
                            );
    propertiesHeader.style.fontSize = 'large';
    propertiesCol.appendChild(propertiesHeader);

    let propertiesContainer = CreateDOMElement('div', {classList: ['container-fluid', 'overflow-auto']});
    ...
    _.forEach(soData.editorData.details.properties, (property) => {
        RenderSmartObjectProperty(
            ...
            soData.editorData.details.mapPropsAlias[property.name],
            {
                ...
                onEditPropertyProgrammingActive: callbacksMap.onEditPropertyProgrammingActive
            }
        );
    });
    ...
    let groupsName = CreateDOMElement(
                        'div', 
                        {
    ...
    let exportGroupsButtonCol = CreateDOMElement('div', {classList: ['col-sm']});
    groupsHeaderRow.appendChild(exportGroupsButtonCol);

    let exportGroupsButton = CreateDOMElement(
                                'button', 
                                {
    ...
    exportGroupsButtonCol.addEventListener('click', ()=> {callbacksMap.onCreateSmartGroup()});    
...
export function RenderSmartObject(selector, soData, callbacksMap) {
    RenderSmartObjectDispatchFunc[soData.editorData.details.state] (selector, soData, callbacksMap);
}
...
let RenderSmartGroupProperty = function(selector, property, alias, callbacks){
    ...
    let propertyActiveIcon = CreateDOMElement('i', {classList: ['fas', 'fa-power-off', 'fa-lg']});
    propertyActiveIcon.addEventListener('click', () => {callbacks.onEditPropertyActive(property)});
...
let RenderSmartObjectofGroup = function(selector, smartObject, onDeleteSmartObject){
    let smartObjectName = CreateDOMElement(
                            'span', 
                            {
                                classList: ['badge', 'badge-pill', 'badge-secondary'], 
    ...
    let smartObjectIcon = CreateDOMElement('i', {classList: ['fas', 'fa-times-circle']});
    smartObjectIcon.addEventListener('click', () => {onDeleteSmartObject(smartObject)});
...
export function RenderSmartGroup(selector, soData, callbacksMap) {
    ...
    let groupIcon = CreateDOMElement('i', {classList: ['fas', 'fa-layer-group', 'fa-lg']});
    groupIconSpan.appendChild(groupIcon);
    ...
    let resetButton = CreateDOMElement(
                        'button', 
                        {
    ...
    let smartObjectsName = CreateDOMElement(
                            'div', 
                            {
    ...
    let deleteGroupButton = CreateDOMElement(
                                'button', 
                                {
                                    classList: ['btn', 'btn-danger'], 
