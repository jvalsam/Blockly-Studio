        ...
        this.redraw_node = function(obj, deep, callback, force_render) {
			...
			if(obj) {
				...
				if(tmp) {
					...
					var highlighted = this._model.data[obj.id].highlighted;
					if (options) {
						let ellipsisId = "options_" + obj.id;
						let ellipsisHTML = createElementFromHTML(
							"<i class=\"fa fa-ellipsis-v fa-xs\" id=\""
							+ ellipsisId
							+ "\" aria-hidden=\"true\" style=\"color: "
							+ highlighted.options.color
							+ "; float: right; margin-top: 6px;\"></i>"
						);
						tmp.appendChild(ellipsisHTML);
						tmp.style.cssText = "width: 90%";

						ellitem = tmp.querySelector("#"+ ellipsisId);
						ellitem.addEventListener(
							"click",
							function (e) {
								$("#" + obj.id + "_anchor")
									.trigger({
										type: 'contextmenu.jstree',
										pageX: e.pageX,
										pageY: e.pageY
									});
								e.preventDefault();
